...
class Layers(object):
    ...
    def denseV2( self, scope, x, output_dim, activation=None):
        return tf.contrib.layers.fully_connected( x, output_dim, activation_fn=activation, reuse = tf.AUTO_REUSE, scope=scope)

    def dense(self, scope, x, output_dim, activation=None, return_param=False):
        ...
        with tf.variable_scope(scope, reuse = tf.AUTO_REUSE):
            W, b = self.Wb([x.get_shape()[1], output_dim], [output_dim])
            o = tf.matmul(x, W) + b 
    ...
    def vae_sampler_new( self, scope, x, size, activation=tf.nn.elu):
        # for LVAE
        with tf.variable_scope(scope, reuse = tf.AUTO_REUSE): 
            mu       = self.dense(scope+'_vae_mu', x, size, activation)
            logsigma = self.dense(scope+'_vae_logsigma', x, size)
            logsigma = tf.clip_by_value(logsigma, eps, 50)
        sigma = tf.exp(logsigma)
        return self.sampler(mu, sigma), mu, logsigma  
    ...
    def bn(self, scope, x, is_train=True, collections=None, name="bn", decay=0.999):
        ...
        def if_true():
            ...
            with tf.control_dependencies([avg_mean_assign_op, avg_var_assign_op]):
                return (x - mean) / tf.sqrt(1e-6 + var)

        def if_false():
            return (x - avg_mean) / tf.sqrt(1e-6 + avg_var)

        z = tf.cond(is_train, if_true, if_false)
